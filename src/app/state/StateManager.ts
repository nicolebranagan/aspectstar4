...
export const setStateFromString: (string: string) => void = string => {
...
] = () => [state.maxLevel, state.maxRow, state.levelMarkers];
...
export const setLevelMarkers = (
  ...
  if (state.levelMarkers[row][level]) {
    ...
    markers = [
      oldMarkers[0] || markers[0],
      oldMarkers[1] || markers[1],
      oldMarkers[2] || markers[2]
    ];
  }
