...
export default {
  getAspectEffect(master: UpdatableHolder, asp: Aspect): Updatable {
    ...
    for (let i = -6; i <= 6; i++) {
      for (let j = 0; j < 3; j++) {
        const particle1 = new ColorParticle(
          asp,
          i,
          -16 + (i % 4),
          24 - Math.abs(i),
        ...
        const particle2 = new ColorParticle(
          asp,
          i,
          -16 + (i % 4),
          24 - Math.abs(i),
...
class ColorParticle {
  ...
  constructor(
    ...
    this.texture = PIXI.Texture.from(
      PIXI.loader.resources["particle"].texture.baseTexture
    );
...
class ImageParticle {
  ...
  constructor(
    ...
    this.texture = PIXI.Texture.from(
      PIXI.loader.resources["particle2"].texture.baseTexture
    );
    this.sprite.texture = this.texture;
    this.determineFrame();
  }
