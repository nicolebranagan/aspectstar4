...
export default class Palace implements Background {
  ...
  drawFixedBackground() {
    this.drawables.addChild(
      this.generateRow(STARRY_SKY, -16 + 48 + 32 + 16, 32)
    );
  }
  ...
  initializeRows() {
    ...
    this.rows.push(
      this.generateRow(LARGE_BRICKS, -16 + 48 + 32 + 16 + 48 + 16)
    );
    this.rows.push(
      this.generateRow(GIANT_BRICKS, -16 + 48 + 32 + 16 + 48 + 16 + 32)
    );
  ...
  generateRow(rect: number[], y: number, width: number = 96) {
    ...
    for (let i = -1; i < Math.ceil(width + 400 / width); i++) {
      centerPillars.addChild(generateSprite(rect, width * i, y));
    }
