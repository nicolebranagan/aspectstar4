...
export default class PlatformerPhysics implements Physics {
  ...
  inrange(point: Point, other: Point) {
    return (
      Math.abs(point.x - other.x) < this.width &&
      Math.abs(point.y - other.y) < this.height
    );
  }

  public step(point: Point, asp: Aspect): Point {
    if (this.isSolid(point, asp)) return point;
  ...
  private isSolid(pt: Point, asp: Aspect): boolean {
    ...
    if (this.xvel < 0)
      checks.push([
        new Point(pt.x - this.width / 2, pt.y - this.height / 2),
        true
      ]);
    if (this.xvel > 0)
      checks.push([
    ...
    if (this.yvel > 0 || this.xvel < 0)
      checks.push([new Point(pt.x - this.width / 2, pt.y), upwardMomentum]);
    if (this.yvel > 0 || this.xvel > 0)
      checks.push([new Point(pt.x + this.width / 2, pt.y), upwardMomentum]);
    if (this.yvel < 0 || this.xvel < 0)
    ...
    return checks
      .map(e => {
        return this.stage.isSolid(e[0], asp, e[1]);
      })
      .some(e => {
        return e;
      });
